...
class Test_1_5_9 extends TestMetadata {
  constructor(metadata) {
    ...
    this.num = '1.5.9';
    this.description =
      "If present, the token_endpoint_auth_signing_alg_values_supported  MUST be  ['RS256', 'RS512']";
  ...
  async exec() {
    ...
    if (
      !(
        this.metadata.configuration
          .token_endpoint_auth_signing_alg_values_supported.length == 2 &&
        this.metadata.configuration.token_endpoint_auth_signing_alg_values_supported.includes(
          'RS256'
        ) &&
        this.metadata.configuration.token_endpoint_auth_signing_alg_values_supported.includes(
          'RS512'
